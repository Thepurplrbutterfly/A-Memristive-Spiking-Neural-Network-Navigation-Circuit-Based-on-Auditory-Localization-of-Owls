...
def main():
    ...
    parser.add_argument('--source_target', type=list, default=[6.9, 5.5, 6.5], help='默认声源位置')
    parser.add_argument('--space_size', type=list, default=[10, 10, 10], help='三维空间大小')
    parser.add_argument('--c0', type=float, default=0.2, help='权重初始化赋1比重')
    parser.add_argument('--dt_step', type=int, default=1, help='时间步长')
    parser.add_argument('--m_step', type=int, default=0.5, help='Agent移动步长')
    parser.add_argument('--gamma_min', type=float, default=0.9,help='最小学习率')
            ...
            for t in trange(int(T_max/T_v), desc=f'第{R+1}次重复实验中的第{Tr+1}次独立实验'):
                ...
                if n_esti_dist > 1:
                    if n_esti_dist <= o_esti_dist:
                        ...
                        Reward_value = -1.5
                elif n_esti_dist >= 0 and n_esti_dist <= 1:
                    arr_flag = 1
                    Reward_value = 0
                ...
                #gamma = gamma_min + (gamma_max - gamma_min) *  (1 - math.exp(3.5 - n_esti_dist))
