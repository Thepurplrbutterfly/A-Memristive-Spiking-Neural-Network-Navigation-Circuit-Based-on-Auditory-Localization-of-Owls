...
def main():
    ...
    parser.add_argument('--source_target', type=list, default=[19.1, 22.8, 20.9], help='默认声源位置')
    ...
    parser.add_argument('--m_step', type=int, default=3.5, help='Agent移动步长')
    parser.add_argument('--gamma_min', type=float, default=1.2,help='最小学习率')
    parser.add_argument('--gamma_max', type=float, default=1.9,help='最大学习率')
    ...
    #设置4个起始点
    _4starting_point = {0:[1,1,1],1:[space_size[0]-1,1,1],2:[1,1,space_size[2]-1],3:[space_size[0]-1,1,space_size[2]-1]}
    #-----------------关键参数-----------------# 
...
# 绘制热力图
    ...
    for R in range(repeat_times):
        ...
        for Tr in range(Trial):  #每次重复实验中对Trial次独立实验进行遍历
            ...
            """
            #衰减因子衰减率变化函数
            ini_dis = math.sqrt((source_target[0]-x_ini)**2 + (source_target[1]-y_ini)**2 + (source_target[2]-z_ini)**2)
            if ini_dis < 15:
                decay_rate = 0.8
            elif ini_dis >= 15 and ini_dis <= 60:
                decay_rate = np.round(0.79 + (0.2 / (1 + math.exp(-0.05 * (ini_dis - 25)))), 3)
            elif ini_dis > 60:
                decay_rate = 0.995
            ...
            for t in trange(int(T_max/T_v), desc=f'第{R+1}次重复实验中的第{Tr+1}次独立实验'):
                ...
                if n_esti_dist > 4.5:
                    if n_esti_dist < o_esti_dist:
                        ...
                        Reward_value = -1
                elif n_esti_dist >= 0 and n_esti_dist <= 4.5:
                    arr_flag = 1
                    Reward_value = 0
                ...
                gamma = gamma_min + (gamma_max - gamma_min) * (abs(n_esti_dist-4.5) / (1 + abs(n_esti_dist-4.5)))
