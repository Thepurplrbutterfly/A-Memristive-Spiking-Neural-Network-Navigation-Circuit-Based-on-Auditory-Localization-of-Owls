...
def main():
    ...
    parser.add_argument('--source_target', type=list, default=[30.9, 35.8, 19.9], help='默认声源位置')
    ...
    parser.add_argument('--gamma_min', type=float, default=1.5,help='最小学习率')
    parser.add_argument('--gamma_max', type=float, default=2.2,help='最大学习率')
        ...
        for Tr in range(Trial):  #每次重复实验中对Trial次独立实验进行遍历
            ...
            for t in trange(int(T_max/T_v), desc=f'第{R+1}次重复实验中的第{Tr+1}次独立实验'):
                ...
                if esti_dist > 4.5: 
                    ...
                    if theta_h == pi/2 or theta_h == -pi/2:
                        ...
                        if x_Agent > space_size[0]-1:
                            x_Agent = (space_size[0]-1) + (m_step * cos(theta_v)) * (-sin(theta_h))
                            front_sensor_pos[0] = x_Agent
                            back_sensor_pos[0] = x_Agent
                            Reward_value = -1
                        if y_Agent > space_size[1]-1:
                            y_Agent = (space_size[1]-1) + (m_step * cos(theta_v)) * (-cos(theta_h))
                            front_sensor_pos[1] = y_Agent + 1
                            back_sensor_pos[1] = y_Agent - 1
                            Reward_value = -1
                        if z_Agent > space_size[2]-1:
                            z_Agent = (space_size[2]-1) + m_step * (-sin(theta_v))
                            front_sensor_pos[2] = z_Agent 
                            back_sensor_pos[2] = z_Agent 
                            Reward_value = -1
                        if x_Agent < 1:
                            x_Agent = 1 + (m_step * cos(theta_v)) * (-sin(theta_h))
                            front_sensor_pos[0] = x_Agent
                            back_sensor_pos[0] = x_Agent
                            Reward_value = -1
                        if y_Agent < 1:
                            y_Agent = 1 + (m_step * cos(theta_v)) * (-cos(theta_h))
                            front_sensor_pos[1] = y_Agent + 1
                            back_sensor_pos[1] = y_Agent - 1
                            Reward_value = -1
                        if z_Agent < 1:
                            z_Agent = 1 + m_step * (-sin(theta_v))
                            front_sensor_pos[2] = z_Agent
                            back_sensor_pos[2] = z_Agent
                            Reward_value = -1
                ...
                if Reward_value != 0:
                    ...
                    W_v_new = W_v + delta_W_v * (Reward_value*1.5) * gamma * decay
