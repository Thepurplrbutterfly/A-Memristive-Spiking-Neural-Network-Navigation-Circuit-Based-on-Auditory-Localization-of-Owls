...
def calculate_runner(fun):
    sdk = fun.__self__
    def fix_calc(input, addr, it_time, data_type, expand_mode, ret_mode):
        output_rram = fun(input, addr, it_time, data_type, expand_mode, ret_mode)
        print("addr:",addr)
        if sdk.adc_cal_en:
            rowstart, colstart, rowcount, colcount = addr
            weight_data = sdk.weight_int4[rowstart:rowstart+rowcount, colstart:colstart+colcount]
        else:
            weight_data = sdk.get_weight_int4(addr, verbose=0)
        output_cpu = np.dot(input, weight_data)
        # 计算相关系数
        # y = output_rram.ravel()
        # x = output_cpu.ravel()
        y = output_cpu.ravel()
        x = output_rram.ravel()
        rram_cpu = np.r_[[y], [x]]
        r_rram_cpu = np.corrcoef(rram_cpu)[0, 1]
        print(f'相关系数:{r_rram_cpu:.4f}')
        # 计算回归系数
        slope, intercept = np.polyfit(x, y, 1)
        print(f"拟合结果: y = {slope:.4f} * x + {intercept:.4f}")
        # 线性拟合到cpu计算结果
        # output_rram = np.round(output_rram*slope + intercept)
        # output_rram = output_rram*slope + intercept
        # print(output_rram[0])
        return output_rram

    #print("my calculate_runner")
    return fix_calc
...
def mvm_bitwise_concat_push_fast_144k(sdk, input, addr, repeat = None, it_time = 5, verbose = 0):
    ...
    # output_bitwise = sdk.calculate(input.transpose(1, 0), addr = addr, runner = calculate_runner, it_time = it_time, data_type = -1, expand_mode = 1)
    output_bitwise = sdk.calculate(input.transpose(1, 0), addr = addr, it_time = it_time, data_type = -1, expand_mode = 1)
    output = output_bitwise
    ...
    if repeat:
        ...
        # TODO: delete
        # This is unnecessary, because the calculation of row direction has integral time to adjust the ADC range, there is no need for averaging
